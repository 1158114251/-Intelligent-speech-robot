...
static int smart_chat(char* send_text, char *recv_text, int recv_text_size, char *code, int recv_code_size)
{
	/*步骤1：创建socket*/
        ...
        int params_len = strlen("key=35ff2856b55e4a7f9eeb86e3437e23fe&info=") + strlen(send_text);
...
static void voice_recognition_control()
{
	...
	*/
	...
	else if (arecord_pid == 0)  //子进程执行录制音频
	{
		...
		if (execlp("/bin/bash", "/bin/bash", "-c", "arecord -d 4 -r 16000 -c 1 -t wav -f S16_LE -D plughw:1,0 /home/pi/VoiceRecognitionControlRobot/wav/cmd_voice_temp/voice_cmd.wav", NULL) < 0)
                { 
	...
	else if (arecord_pid > 0)   //父进程执行
	{
		...
		if (strstr(voice_recongnition_cmd, "开"))
		{
			show_sys_info("语音控制：正在开灯...\n");
			system("python open_light.py&");
			//这个地方添加控制函数
                        system("mplayer wav/open_light.mp3");
		}
		else if (strstr(voice_recongnition_cmd, "关"))
		{
			show_sys_info("语音控制：正在关灯...\n");
			system("python close_light.py&");
                        system("mplayer wav/close_light.mp3");

                	//cam_turn_point(0, 0);
			//这个地方添加控制函数
		}
		else if (strstr(voice_recongnition_cmd, "舞"))
		{
			show_sys_info("语音控制：跳舞...");
			//cam_turn_point(-90, 0);
                        system("./a.out");
		}
...
char * url=te;                                       
     ...
     sprintf(url,"mpg123 \"http://tsn.baidu.com/text2audio?tex=%s&lan=zh&per=0&pit=1&spd=7&cui\
d=7519663&ctp=1&tok=25.41bf315625c68b3e947c49b90788532d.315360000.179\
8261651.282335-9120612\"",recv_text);
